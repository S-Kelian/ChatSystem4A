...
public class ContactList {
    ...
    public void create() {
        ...
        JFrame frame = new JFrame("ContactList");
    ...
    private void createButtonsOfUsers(JPanel panelUsersOnline) {
        ...
        for (int i = 0; i < app.getMyUserList().getUsersOnline().size(); i++) {
            ...
            // Add "(you)" to the label of the current user and add an event to the button to other users
            if (app.getMyUserList().getUsersOnline().get(i).getNickname().equals(app.getMe().getNickname())) {
                ...
                userButton.addActionListener(e -> {
                    // Ask the user if he wants to open a chat with the selected user
                    int askResponse = ((int) (Math.random() * 10)%2);
                    if (askResponse == 0) {
                        // Open a chat with the selected user

                    } else if (askResponse == 1) {
                        JOptionPane.showMessageDialog(null, userButton.getText() + " is refusing to chat with you", "Refused", JOptionPane.INFORMATION_MESSAGE);
                    }
                });
            }
